<?php
use Pokemons\CustomPokemons;
class PokemonArchive{
    ...
    function poks_archive_output() {
        ...
        foreach ($filtered_poks as $pok) {
            ...
            echo '<div class="grid_item">';
                echo '<div class="slider_wrap">';
                    echo CustomPokemons::poks_markup($pok->image, $pok->maxHP, $pok->maxCP, $pok->fleeRate, $pok->name);
                    if ($evolutions !== null) {
                        foreach ($evolutions as $evo_pok) {
                            echo CustomPokemons::poks_markup($evo_pok->image, $evo_pok->maxHP, $evo_pok->maxCP, $evo_pok->fleeRate, $evo_pok->name);
                        }
                    }
                echo'</div>';
            echo '</div>';
        ...
        ?>
            <div id="show_more" class="pokemon_cont show_more">
                <span><?php echo __('Show More'); ?></span>
            </div>
        </div>
        <?php
        $out = ob_get_clean();
 ...
 * */
if (CustomPokemons::current_page_id('%[pokemons_archive%')) {
    new PokemonArchive();
}